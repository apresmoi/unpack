...
1. **Read the guide configuration** from `docs/_meta/guide-config.md` (if it exists):
...
2. **If human docs are disabled**: inform the user and suggest they update `docs/_meta/guide-config.md` if they want to enable them. Stop here.

3. **Read the current project state**:
   - `docs/index.md`
   - All `docs/specs/*`
   - `docs/_meta/project-memory.md`
   - Relevant `docs/phases/*` (completed phases show what was built)
...
6. **Writing style**:
   ...
   - Reference specs for canonical detail: "For the full specification, see `docs/specs/03-architecture.md`"
...
- Human docs (`guide/`) are SEPARATE from agent docs (`docs/`)
- `docs/` is the source of truth for agents — `guide/` is the explanation layer for humans
