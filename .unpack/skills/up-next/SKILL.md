...
1. **Read first** (mandatory before any work):
   ...
   - `.unpack/docs/practices/*` (loaded standards for this project)
   - All phase files in `.unpack/docs/phases/`
...
4. **Execute the phase**:
   ...
   - Follow loaded standards in `.unpack/docs/practices/*` for code style, naming, structure
...
6. **Update everything**:
   ...
   - `.unpack/docs/index.md`: phase status table, current focus
   - Any affected specs (if behavior/architecture changed)
   - `.unpack/docs/_meta/project-memory.md`: append new decisions
...
7. **Steering check**: if during implementation you discover scope/architecture/constraint changes:
   ...
   - Update dependencies and `.unpack/docs/index.md`
